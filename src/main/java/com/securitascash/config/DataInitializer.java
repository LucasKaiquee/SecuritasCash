...
public class DataInitializer {
    ...
    @Bean
    CommandLineRunner loadDataUsuarioCorrentista(UsuarioRepository repositorio, PasswordEncoder passwordEncoder) {
        return args -> {
            if (repositorio.count() == 0) {
                repositorio.save(new Administrador(
                        null,
                        "Lucas",
                        passwordEncoder().encode("admin123"), // Senha criptografada
                        "lucas@teste.com",
                        null
                ));
                repositorio.save(new Correntista(
                        null,
                        "Luiz",
